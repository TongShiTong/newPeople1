<template>
  <div class="home-page">
    <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="100" style="overflow: scroll;height: 100vh">
      ...
      <van-tabs class="van_tabs_box" v-model="active" line-width='20' swipe-threshold="5" sticky @click="vanTabs">
        <van-tab title="新人专享">
          <div class="goods_box">
        ...
        <van-tab title="居家日用">
            ...
            <div v-if="!finished" class="van-list__loading"><div class="van-loading van-loading--circular"><span class="van-loading__spinner van-loading__spinner--circular" style="width: 16px; height: 16px;"><svg viewBox="25 25 50 50" class="van-loading__circular"><circle cx="50" cy="50" r="20" fill="none"></circle></svg></span><span class="van-loading__text">加载中...</span></div></div>
            <div v-else class="van-list__finished-text">没有更多了</div>
        </van-tab>
        <van-tab title="美食酒水">
...
<script>
import { setToken, getToken, getUserInfo, setKeyData, getKeyData, getKey, setAgain, getAgain, getfrist1, getfrist2, getfrist3, setfrist1, setfrist2, setfrist3, setLq1, setLq2, setLq3, getLq1, getLq2, getLq3 } from "../store/user";
...
export default {
  ...
  methods: {
    ...
    loadMore() {
      var that = this
      if(this.mcid == 656 || this.mcid == 663 || this.mcid == 648 || this.mcid == 688) {
        if(!that.finished) {
          that.goodList(this.mcid,this.start_page)
        }
      }	
		},
    vanTabs(e) {
    ...
    goodList(mcid, start_page) {
      ...
      if (self.lodingLock) {
		  ...
		  self.lodingLock = true;
      api.goodList(mcid, start_page).then(data => {
        self.lodingLock = false;
        let datas = data.list
    ...
    getItem1(item_id) {
      ...
      if (bridge.hasBridge()) {
        ...
        window.wx.miniProgram.navigateTo({url: `/pages/goodDetail/goodDetail?shop_id=${data.shop_id}&id=${data.item_id}`})
